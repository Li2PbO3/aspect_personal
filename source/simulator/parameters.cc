...
namespace aspect
{
  ...
  declare_parameters (ParameterHandler &prm)
  {
    ...
    {
      prm.declare_entry ("Prescribed Neumann boundary indicators", "",
                         Patterns::Map (Patterns::Anything(),
                                        Patterns::Selection(BoundaryNeumannCondition::get_names<dim>())),
                         "A comma separated list denoting those boundaries "
                         "on which a Neumann boundary condition is prescribed, i.e., where "
                         "known fluxes of temperature or composition are prescribed. This is "
                         "often used to model ``open'' boundaries where we only know the fluxes. "
                         "The format of valid entries for this parameter is that of a map "
                         "given as ``key1 [selector]: value1, key2 [selector]: value2, key3: value3, ...'' where "
                         "each key must be a valid boundary indicator (which is either an "
                         "integer or the symbolic name the geometry model in use may have "
                         "provided for this part of the boundary) "
                         "and each value must be one of the currently implemented Neumann boundary "
                         "condition models. ``selector'' is an optional string given as a subset "
                         "of the letters `xyz' that allows you to apply the boundary conditions "
                         "only to the components listed. As an example, '1 y: function' applies "
                         "the type `function' to the y component on boundary 1. Without a selector "
                         "it will affect all components of the Neumann boundary condition.");
    }
    prm.leave_subsection();
                                      ...
                                      const GeometryModel::Interface<dim> &geometry_model)
  {
    ...
    prm.enter_subsection ("Boundary Neumann condition model");
    {
      const std::vector<std::string> x_prescribed_Neumann_boundary_indicators
        = Utilities::split_string_list
          (prm.get ("Prescribed Neumann boundary indicators"));
      for (const auto &p : x_prescribed_Neumann_boundary_indicators)
        {
          // each entry has the format (white space is optional):
          // <id> [x][y][z] : <value (might have spaces)>
          //
          ...
          const std::vector<std::string> split_parts = Utilities::split_string_list (p, ':');
          AssertThrow (split_parts.size() == 2,
                       ExcMessage ("The format for prescribed Neumann boundary indicators "
                                   "requires that each entry has the form `"
                                   "<id> [x][y][z] : <value>', but there does not "
                                   "appear to be a colon in the entry <"
                                   + p
          ...
          //   as a component selector, and we better undo it
          // - the last character of key_and_comp is not a whitespace. this
          //   means that the last word in key_and_comp ended in an 'x', 'y' or 'z', 
          //   but this was not meant to be a component selector.
          //   in that case, put these characters back.
          // - otherwise, we split successfully. eat spaces that may be at
          //   the end of key_and_comp to get key
          if (key_and_comp.size() == 0)
          ...
          // finally, try to translate the key into a boundary_id.
          // then make sure we haven't seen it yet
          types::boundary_id boundary_id;
          try
            {
              boundary_id = geometry_model.translate_symbolic_boundary_name_to_id(key_and_comp);
            ...
            {
              AssertThrow (false, ExcMessage ("While parsing the entry <Boundary Neumann condition model/Prescribed "
                                              "Neumann boundary indicators>, there was an error. Specifically, "
                                              "the conversion function complained as follows:\n\n"
                                              + error));
            }
          AssertThrow (prescribed_Neumann_boundary_indicators.find(boundary_id)
                       == prescribed_Neumann_boundary_indicators.end(),
                       ExcMessage ("Boundary indicator <" + Utilities::int_to_string(boundary_id) +
                                   "> appears more than once in the list of indicators "
                                   "for nonzero Neumann boundaries."));
          // finally, put it into the list
          prescribed_Neumann_boundary_indicators[boundary_id] =
            std::pair<std::string,std::string>(comp,value);
        }
    }
    prm.leave_subsection ();
